...
const UploadFile: React.FC<UploadFileProps> = ({ onFileUpload,file }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-1 w-[170px]">
      <label
        htmlFor="uploadFile1"
        className={`flex bg-gray-600 hover:bg-gray-700 text-white text-base font-medium px-4 py-2.5 outline-none rounded w-max cursor-pointer ${
          file ? 'hidden' : ''
        }`}
      >
      ...
      {file ? (
        ...
        <p className={`text-sm pl-2.5 ${error ? 'text-red-500' : 'text-gray-500'}`}>{error || 'Please upload a .rob file'}</p>
      )}
    </div>
  );
};
